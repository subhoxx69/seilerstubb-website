...
export default function AnnouncementsPage() {
  ...
  const { user, isLoading: authLoading, firebaseUser } = useAuth();
  ...
  useEffect(() => {
    if (!authLoading) {
      const adminCheck = firebaseUser && isAdminUser(firebaseUser.email);
  ...
  const handleSave = async () => {
    try {
      ...
      setError('Fehler beim Speichern der Ankündigung: ' + (err instanceof Error ? err.message : 'Unbekannt'));
    } finally {
      setIsSaving(false);
    }
  };
