...
export async function logoutAllUsers(userEmail: string) {
  try {
    // Verify actor is main admin
    const isMainAdmin = MAIN_ADMIN_EMAILS.includes(userEmail);

    if (!isMainAdmin) {
      return { success: false, error: 'Unauthorized: Only main admins can log out all users' };
    }
    ...
    for (const userDoc of usersSnap.docs) {
      ...
      sessionsSnap.docs.forEach((sessionDoc) => {
        batch.update(sessionDoc.ref, { revoked: true });
        invalidatedCount++;
      });
    }
    ...
    await logAdminAction(userEmail, 'ALL_USERS', 'logout_all', { invalidatedSessions: invalidatedCount });

    return {
      success: true,
      message: `All users have been logged out successfully (${invalidatedCount} sessions invalidated)`,
      invalidatedCount,
    };
  } catch (error: any) {
    ...
    return {
      success: false,
      error: error?.message || 'Failed to log out all users',
    };
  }
}
